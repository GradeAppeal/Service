...
CREATE OR REPLACE PROCEDURE proc_invoke_digest_email(pid uuid, email text, digest_type text) LANGUAGE plpgsql SECURITY DEFINER
SET search_path = PUBLIC AS $$
DECLARE number_of_appeals text;
BEGIN -- get number of appeals whenever sending the email
SELECT COUNT(*)::text INTO number_of_appeals
FROM PUBLIC."Appeals"
WHERE created_at >= CURRENT_TIMESTAMP - digest_type::interval
