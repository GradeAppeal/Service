CREATE OR REPLACE FUNCTION on_schedule_cron() RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER
SET search_path = PUBLIC AS $$
DECLARE job_id bigint;
job_name text;
digest_type text;
-- only schedule cron job if the professor is verified
BEGIN IF NEW.is_verified = TRUE THEN job_name := NEW.first_name || '-' || NEW.last_name || '-Digest';
CASE
...
ELSE RAISE EXCEPTION 'Unknown cron job pattern: %',
NEW.cron_job;
END CASE
;
